...
object EitherExercises {
  ...
  def createPerson2(name: String, age: String): Either[AppError, Person] = {
    ...
    errorOrName.flatMap(name => errorOrAge.map(age => Person(name, age)))
  }
  ...
  def createPersonAndShow(name: String, age: String): String = {
    val errorOrPerson: Either[AppError, Person] = createPerson(name, age)
    errorOrPerson match {
      case Left(EmptyName(_)) => "Empty name supplied"
      case Left(InvalidAgeRange(_)) => "Invalid age value supplied"
      case Left(InvalidAgeValue(_)) => "Invalid age range supplied"
  ...
  def createValidPeople: List[Person] = {
    val listOfErrorOrPerson: List[Either[AppError, Person]] = personStringPairs.map {
      case (name, age) => createPerson(name, age)
    }
    listOfErrorOrPerson.collect {
